# -*- coding: utf-8 -*-
# ---------------------------------------------------------------------------
# Random Forest Model and Projection Analysis
...
# Description: This tool provides a Random Forest model and projection with variable selection and best model fit determined using data-driven approaches.
# ---------------------------------------------------------------------------
...
Predictor_Folder_Future <- as.vector(Arguments[,2])
Species_Shapefile <- as.vector(Arguments[,3])
Output_Raster_Current <- as.vector(Arguments[,4])
Output_Raster_Future <- as.vector(Arguments[,5])
Bootstrap <- as.numeric(as.vector(Arguments[,6]))
Permutations <- as.numeric(as.vector(Arguments[,7]))
...
# Create future raster stack for model projection.
...
# Predict future distribution raster.
cat("Exporting future distribution raster...\n")
Raster_Variables_Future <- stack(paste(Predictor_Folder_Future, paste(rownames(Final_Fit$importance), "img", sep="."), sep="/"))
predict(Raster_Variables_Future, Final_Fit, Output_Raster_Future, type = "prob", index = 2, na.rm = TRUE, overwrite = TRUE, progress = "window")