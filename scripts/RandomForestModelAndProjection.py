# -*- coding: utf-8 -*-
# ---------------------------------------------------------------------------
# Random Forest Model and Projection Pre/Post-Processing
# Author: Timm Nawrocki, Alaska Center for Conservation Science
# Created on: 2017-01-22
# Usage: Must be executed as an ArcPy Script.
# Description: This tool provides a Random Forest species distribution model that runs from ArcGIS for a single current or historic timeframe and a single future timeframe.
# ---------------------------------------------------------------------------

# Import arcpy, os, and subprocess modules
import arcpy, os, subprocess

# Set overwrite option
arcpy.env.overwriteOutput = True

# Define folder of current environmental predictor raster datasets
Predictor_Folder_Current = arcpy.GetParameterAsText(0)

# Define folder of future environmental predictor raster datasets
Predictor_Folder_Future = arcpy.GetParameterAsText(1)

# Define input species presence/absence points
Species_Shapefile = arcpy.GetParameterAsText(2)

# Define current output raster
Output_Raster_Current = arcpy.GetParameterAsText(3)

# Define future output raster
Output_Raster_Future = arcpy.GetParameterAsText(4)

# Define output probability threshold file
Probability_Threshold = arcpy.GetParameterAsText(5)

# Define location of RScript.exe
RScript_Program = arcpy.GetParameterAsText(6)

# Define location of R Script
RandomForest_Script = arcpy.GetParameterAsText(7)

# Set number of bootstrap replicates
Bootstrap = arcpy.GetParameterAsText(8)

# Set number of validation permutations
Permutations = arcpy.GetParameterAsText(9)
...
# Prepare arguments for predictor folder paths
...
Predictor_Folder_Path_Future = str(Predictor_Folder_Future)
Predictor_Folder_Path_Future = "'" + Predictor_Folder_Path_Future + "'"
...
# Prepare argument for future 64-bit distribution raster output
Output_Name_Future = os.path.split(Output_Raster_Future)[1]
Output_Name_Future = os.path.splitext(Output_Name_Future)[0]
Filename_Future = Output_Name_Future + "_64bit.img"
Output_Raster_64_Future = os.path.join(Workspace, Filename_Future)
Output_Raster_64_Future_Path = str(Output_Raster_64_Future)
Output_Raster_64_Future_Path = "'" + Output_Raster_64_Future_Path + "'"

# Prepare arguments for workspace
Workspace_Path = str(Workspace)

# Write arguments to text file
text_file = open(Arguments, "w")
text_file.write(Predictor_Folder_Path_Current + " " + Predictor_Folder_Path_Future + " " + Species_Shapefile_Path + " " + Output_Raster_64_Current_Path + " " + Output_Raster_64_Future_Path + " " + Bootstrap + " " + Permutations)
text_file.close()
...
# Define projection for current and future outputs
...
arcpy.DefineProjection_management(Output_Raster_64_Future, Projection)
...
arcpy.CopyRaster_management(Output_Raster_64_Future, Output_Raster_Future, "", "", "-3.402823e+038", "NONE", "NONE", "32_BIT_FLOAT", "NONE", "NONE")
...
arcpy.Delete_management(Output_Raster_64_Future)
os.remove(Arguments)
